import { NextResponse } from "next/server";
import { structureIdea } from "../../../lib/ai/structure-idea";
import { createSupabaseServerClient } from "../../../lib/supabase/server";
import {
  getAuthenticatedOwnerId,
  parseInboxCaptureInput,
  saveInboxCapture,
} from "../../inbox/capture";

export const dynamic = "force-dynamic";

export async function POST(request: Request) {
  const supabase = await createSupabaseServerClient();
  const { data } = await supabase.auth.getClaims();
  const ownerId = getAuthenticatedOwnerId(data?.claims ?? null);

  if (!ownerId) {
    return NextResponse.json(
      {
        message: "Your session expired. Sign in again to save ideas.",
      },
      { status: 401 },
    );
  }

  let payload: unknown;
  try {
    payload = await request.json();
  } catch {
    return NextResponse.json(
      {
        message: "Send the idea as JSON.",
      },
      { status: 400 },
    );
  }

  const draft = parseInboxCaptureInput(payload);
  const result = await saveInboxCapture({
    claims: { sub: ownerId },
    draft,
    store: {
      async createIdea(record) {
        const { data: inserted, error } = await supabase
          .from("ideas")
          .insert(record)
          .select("id, created_at")
          .single();

        if (error || !inserted) {
          throw error ?? new Error("Unable to save your idea right now.");
        }

        return inserted;
      },
    },
  });

  if (!result.ok) {
    return NextResponse.json(
      {
        fieldErrors: result.fieldErrors ?? {},
        message: result.message,
      },
      { status: result.status },
    );
  }

  if (draft.intent === "raw") {
    return NextResponse.json(
      {
        createdAt: result.createdAt,
        ideaId: result.ideaId,
        message: result.message,
        structuringPending: false,
        structuringStatus: "not_requested",
      },
      { status: 201 },
    );
  }

  try {
    const outcome = await structureIdea(result.ideaId);

    if (outcome.status === "structured") {
      return NextResponse.json(
        {
          createdAt: result.createdAt,
          ideaId: result.ideaId,
          message: "Saved raw idea and structured it.",
          structuringPending: false,
          structuringStatus: "structured",
        },
        { status: 201 },
      );
    }

    return NextResponse.json(
      {
        createdAt: result.createdAt,
        ideaId: result.ideaId,
        message:
          outcome.status === "failed"
            ? `Saved raw idea. Structuring failed (${outcome.reason}); the saved idea can be retried.`
            : "Saved raw idea. Structuring could not find the saved idea; retry is available.",
        structuringPending: true,
        structuringStatus: "failed",
      },
      { status: 201 },
    );
  } catch {
    return NextResponse.json(
      {
        createdAt: result.createdAt,
        ideaId: result.ideaId,
        message: "Saved raw idea. Structuring is unavailable right now; the saved idea can be retried.",
        structuringPending: true,
        structuringStatus: "failed",
      },
      { status: 201 },
    );
  }
}
